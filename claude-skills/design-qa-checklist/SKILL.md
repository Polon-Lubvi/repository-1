...
description: Сверка собранного интерфейса с макетом — чек-лист для ревью реализации против дизайна. Использовать ТОЛЬКО когда есть макет или Figma и надо проверить, что вёрстка ему соответствует: «совпадает ли с макетом», «сверь с дизайном», «дизайн-ревью». Для проверки сайта без макета — qa-web-sweep, для тест-планов и багов — qa-test-planner.
---
# Design QA Checklist

> **Границы.** Этот скилл — только про сверку с макетом. Всё механическое — контраст по WCAG,
> тапабельные элементы, горизонтальный скролл, битые ссылки, ARIA, фокус, prefers-reduced-motion —
> уже покрыто `qa-web-sweep`, и там это исполняемые пробы, а не список. Процесс QA, приоритеты
> и формат баг-репортов — в `qa-test-planner`. Сюда идти только за сравнением «макет против реализации».
