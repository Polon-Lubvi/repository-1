...
(function () {
  ...
  /* Открыто ли сейчас — по графику из конфига, в часовом поясе сервиса.
     byDay: у каждого дня свой интервал (у субботы короче, воскресенье
     выходной). Старый формат days/from/to тоже понимаем. */
  function isOpenNow() {
    var sc = C.schedule;
    if (!sc) return null;
    ...
    if (sc.byDay) {
      var iv = sc.byDay[d.getDay()];
      if (!iv) return false;
      return mins >= iv[0] && mins < iv[1];
    }
    if (sc.days.indexOf(d.getDay()) === -1) return false;
    return mins >= sc.from && mins < sc.to;
  }

  /* Телефон и мессенджеры одним рядом. Всё ведёт на реальный номер
     или в мессенджер этого номера — «мёртвых» ссылок в ряду нет. */
  function contactLinks() {
    var out = [];
    function pill(href, icon, label, mod) {
      out.push('<a class="cpill' + (mod ? ' cpill--' + mod : '') + '" href="' + esc(href) + '"' +
        (href.indexOf('tel:') === 0 ? '' : ' target="_blank" rel="noopener"') + '>' +
        ic(icon, { size: 16 }) + esc(label) + '</a>');
    }
    if (C.contacts.phone) {
      pill(telLink(), 'phone', C.contacts.phoneDisplay || 'Позвонить', 'tel');
      pill(waLink(), 'whatsapp', 'WhatsApp', 'wa');
    }
    if (viberLink())          pill(viberLink(), 'phone', 'Viber', 'viber');
    if (C.contacts.telegram)  pill(tgLink(), 'telegram', 'Telegram', 'tg');
    if (C.contacts.max)       pill(maxLink(), 'max', 'MAX', 'max');
    if (C.yandex && C.yandex.org) pill(C.yandex.org, 'star', 'Яндекс.Карты', 'ya');
    return out.join('');
  }

  /* Значок бренда: картинка из brand.logo, иначе буквенная плашка */
  function brandMark(size) {
    if (S.brand.logo) {
      return '<img class="logo__img" src="' + esc(S.brand.logo) + '" alt="' + esc(S.brand.name) + '"' +
        ' width="' + size + '" height="' + size + '" style="width:' + size + 'px;height:' + size + 'px">';
    }
    return '<span class="logo__mark">' + esc(S.brand.short) + '</span>';
  }
  ...
  function renderReviews() {
    ...
    /* На сайте показываем только отзывы на пять звёзд — так просил
       заказчик. Отзывы реальные, с карточки организации: имя, уровень
       знатока, дата и текст взяты как есть, ничего не дописано. */
    var items = r.items.map(function (x) {
      ...
      return '<a class="review" href="' + esc(href) + '"' + (href ? ' target="_blank" rel="noopener"' : '') +
        ...
        '<span class="review__foot">' + ic('star', { size: 13 }) + esc(srcLabel) +
          (x.date ? '<span class="review__date">' + esc(x.date) + '</span>' : '') +
        '</span>' +
      '</a>';
    }).join('');
    ...
    return '<section class="reviews" id="reviews"><div class="wrap">' +
      ...
      (r.note ? '<p class="reviews__note">' + esc(r.note) +
        (url ? ' <a href="' + esc(url) + '" target="_blank" rel="noopener">Все отзывы на Яндекс.Картах</a>' : '') +
        '</p>' : '') +
  ...
  function renderLocation() {
    ...
    /* Время в пути по районам. Каждая строка — ссылка «проложить маршрут»
       в Яндекс.Картах от опорной точки района до сервиса. */
    var pt = C.point;
    var routes = l.routes.map(function (r) {
      var href = (pt && r.ll)
        ? 'https://yandex.ru/maps/?rtext=' + encodeURIComponent(r.ll) + '~' +
          encodeURIComponent(pt.lat + ',' + pt.lon) + '&rtt=auto'
        : '';
      var body =
        '<span class="loc__route-from">' + esc(r.from) +
          (r.land ? '<i>' + esc(r.land) + '</i>' : '') + '</span>' +
        '<span class="loc__route-time">' + (r.time ? esc(r.time) : '') +
          (r.km ? '<i>' + esc(r.km) + '</i>' : '') + '</span>';
      return href
        ? '<a class="loc__route is-link" href="' + esc(href) + '" target="_blank" rel="noopener" ' +
          'title="Проложить маршрут в Яндекс.Картах">' + body + '</a>'
        : '<div class="loc__route">' + body + '</div>';
    }).join('');
      ...
      '<div class="loc__grid">' +
        ...
        '<div class="loc__info">' +
          ...
          '<div class="loc__block">' +
            ...
            (l.routesNote ? '<p class="loc__routes-note">' + esc(l.routesNote) + '</p>' : '') +
          '</div>' +
          (l.messengersLabel ? '<div class="loc__block">' +
            '<b>' + esc(l.messengersLabel) + '</b>' +
            '<div class="loc__contacts">' + contactLinks() + '</div>' +
          '</div>' : '') +
  ...
  function renderFooter() {
    ...
    /* Ссылки на организацию: карточка, отзывы, цены, маршрут.
       Показываем только то, что действительно задано в конфиге. */
    var org = [];
    function orgLink(href, label) {
      if (href) org.push('<a href="' + esc(href) + '" target="_blank" rel="noopener">' + esc(label) + '</a>');
    }
    orgLink(y.org, 'Карточка на Яндекс.Картах');
    orgLink(y.reviews, 'Отзывы на Яндекс.Картах');
    orgLink(y.prices, 'Цены на Яндекс.Картах');
    orgLink(y.route, 'Проложить маршрут');
    orgLink(links.avito, 'Авито');

    var hours = (f.schedule || []).map(function (h) {
      return '<span class="footer__day' + (h.off ? ' is-off' : '') + '">' +
        '<i>' + esc(h.days) + '</i>' + esc(h.time) + '</span>';
    }).join('');
    ...
    return '<footer class="footer"><div class="wrap">' +
      ...
      '<div class="footer__top">' +
        '<div class="footer__brand">' + brandMark(44) +
          '<span>' +
            '<b>' + esc(S.brand.name) + '</b>' +
            '<i>' + esc(f.legalNote || legal.orgName || '') + '</i>' +
          '</span>' +
        '</div>' +
        '<div class="footer__contacts">' + contactLinks() + '</div>' +
      '</div>' +
      ...
      '<div class="footer__grid">' +
        ...
        '<div class="footer__col">' +
          '<b>Адрес</b>' +
          '<p class="footer__addr">' + esc(f.address) + '</p>' +
          (y.route ? '<a href="' + esc(y.route) + '" target="_blank" rel="noopener">Проложить маршрут</a>' : '') +
        '</div>' +

        '<div class="footer__col">' +
          '<b>' + esc(f.hoursTitle || 'График работы') + '</b>' +
          '<div class="footer__hours">' + hours + '</div>' +
        '</div>' +

        '<div class="footer__col">' +
          '<b>' + esc(f.linksTitle) + '</b>' +
          (org.join('') || '<span>Ссылки — уточняются</span>') +
        '</div>' +

        '<div class="footer__col">' +
          '<b>Услуги</b>' +
          (f.services || []).map(function (x) {
            return '<a href="#tuning">' + esc(x) + '</a>';
          }).join('') +
        '</div>' +
      ...
      '<div class="footer__bottom">' +
        ...
        '<span class="footer__src">' + esc(f.sources || '') + '</span>' +
      '</div>' +
    '</div></footer>';
  }
  ...
  function wireCalculator() {
    ...
    function recalc() {
      ...
      /* Если газ дороже бензина (бывает при самой левой цене бензина),
         окупаемости нет — так и пишем, а не рисуем деление на ноль. */
      if (saveMonth > 0 && cfg.installPrice) {
        var months = Math.ceil(cfg.installPrice / saveMonth);
        out.payback.textContent = fmt(months) + ' ' + S.calc.paybackUnit;
      } else {
